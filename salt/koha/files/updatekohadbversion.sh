...
if [[ -n "$KOHAVERSION" ]] ; then
  ...
  if [[ ! -z $KOHADBVERSIONOLD && ${KOHADBVERSIONOLD+x} ]] && [[ $KOHADBVERSIONOLD == $KOHADBVERSION ]] ; then
    # Up to date!
    CMD=""
    CHANGED=no
    COMMENT="koha DB ($KOHADBVERSION and KohaOld $KOHADBVERSIONOLD) version already up to date!"
