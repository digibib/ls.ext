...
;(function (root, factory) {
    ...
    _.each(ractive.get('inputGroups'), function (group, groupIndex) {
      _.each(group.inputs, function (input, inputIndex) {
        if (input.searchResult) {
          ractive.set('inputGroups.' + groupIndex + '.inputs.' + inputIndex + '.searchResult', null)
        }
      })
    })
  }
            ...
            _.each(actualRoots, function (root) {
              ...
              if (mainEntryInput === mainEntryNode) {
                if (_.contains(['select-authorized-value', 'entity', 'searchable-authority-dropdown'], input.type)) {
                  var values = setMultiValues(root.outAll(propertyName(predicate)), input, 0)
                  ...
                  setMultiValues(root.outAll(propertyName(predicate)), input, 0)
                } else {
                ...
                if (input.isSubInput) {
                  input.values[0].nonEditable = true
                  input.values[0].subjectType = type
  ...
  function createInputForCompoundInput (prop, tab, ontologyUri, inputMap) {
    ...
    _.each(prop.subInputs.inputs, function (subInput) {
      ...
      var enableCreateNewResource = newSubInput.input.widgetOptions ? newSubInput.input.widgetOptions.enableCreateNewResource : undefined
      if (enableCreateNewResource) {
        _.each(enableCreateNewResource.inputs, function (input) {
          var inputFromOntology = inputMap[enableCreateNewResource.rdfType + '.' + ontologyUri + input.rdfProperty]
          if (!inputFromOntology) {
            throw new Error('Unknown property: ' + enableCreateNewResource.rdfType + '.' + ontologyUri + input.rdfProperty)
          }
          _.extend(input, deepClone(inputFromOntology))
  ...
  var createInputGroups = function (applicationData) {
    ...
    _.each(tabs, function (tab) {
      var group = {}
      var groupInputs = []
      _.each(tab.inputs, function (tabInput, index) {
        var ontologyInput
        if (tabInput.searchMainResource) {
          if (tabInput.widgetOptions.enableCreateNewResource) {
            _.each(tabInput.widgetOptions.enableCreateNewResource.inputs, function (input) {
              input['values'] = emptyValues(false)
              input['rdfType'] = tabInput.widgetOptions.enableCreateNewResource.rdfType
              input['type'] = input.type
              input['preFillFromSearchField'] = input.preFillFromSearchField
              input['predicate'] = ontologyUri + input.rdfProperty
            })
          }
          groupInputs.push({
            label: tabInput.searchMainResource.label,
            values: emptyValues(false, true),
            type: 'searchable-with-result-in-side-panel',
            visible: true,
            indexTypes: [tabInput.searchMainResource.indexType],
            selectedIndexType: tabInput.searchMainResource.indexType,
            searchResult: null,
            isMainEntry: true,
            isRoot: tabInput.searchMainResource.isRoot,
            widgetOptions: tabInput.widgetOptions,
            datatype: tabInput.datatype,
            showOnlyWhenEmpty: tabInput.searchMainResource.showOnlyWhenMissingTargetUri
          })
        } else {
          if (tabInput.rdfProperty) {
            ontologyInput = deepClone(inputMap[tab.rdfType + '.' + ontologyUri + tabInput.rdfProperty])
            if (typeof ontologyInput === 'undefined') {
              throw new Error("Tab '" + tab.label + "' specified unknown property '" + tabInput.rdfProperty + "'")
            }
          } else if (tabInput.subInputs) {
            ontologyInput = createInputForCompoundInput(tabInput, tab, ontologyUri, inputMap)
          } else {
            throw new Error('Input #' + index + " of tab '" + tab.label + "' must have rdfProperty, subInputs or searchMainResource")
          }

          if (tab.rdfType === unPrefix(ontologyInput.domain)) {
            groupInputs.push(ontologyInput)
            ontologyInput.rdfType = tab.rdfType
          }
          transferInputGroupOptions(tabInput, ontologyInput)
          transferWidgetOptions(tabInput, ontologyInput, inputMap, ontologyUri)
        }
      })
    ...
    init: function () {
      ...
      var initRactive = function (applicationData) {
        ...
        ractive.on({
          ...
          showCreateNewResource: function (event) {
            ractive.set(event.keypath + '.showCreateInputs', true)
            var searchTerm = ractive.get(grandParentOf(event.keypath) + '.values.0.current')
          ...
          createNewResource: function (event, origin) {
            ...
            saveInputs(event.context.inputs, event.context.rdfType)
              ...
              ractive.update()
              if (ractive.get(event.keypath).useAfterCreation) {
