    ...
    init: function () {
      ...
      ractive.on({
        ...
        patchResource: function (event, predicate) {
          ...
          axios.patch(ractive.get("resource_uri"),
      ...
      ractive.set("resource_type", StringUtil.titelize(location.pathname.substr(location.pathname.lastIndexOf("/") + 1)));

      // Application entrypoint - start with fetching config
      axios.get("/config", {"Accept": "application/ld+json"})
      .then(function (res) {
        ractive.set("config", JSON.parse(res.responseText));
      })
      .then(function () {
        axios.get(ractive.get("config.ontologyUri"), {"Accept": "application/ld+json"})
        .then(function (res) {
