describe("Given user is logged in", function () {
    ...
    var $httpBackend,
        ...
        configUrl = '/config',
        mocks = {},
        workW222557057913Uri = 'http://192.168.50.12:8005/work/w222557057913',
   ...
   describe("And the page is finished initialized", function () {
       ...
       describe("And the user navigates to /work/w222557057913", function() {
            beforeEach(function () {
                ...
                $httpBackend.expectGET(workW222557057913Uri).respond(200, mocks.work_w222557057913);
                $location.path('/work/w222557057913');
                $rootScope.$apply();
                controller = createController('WorkController', appController.$scope);
            ...
            describe("And the user changes the date", function () {
                ...
                describe("And focus out triggers save (success)", function () {
                    var needsFlush = true,
                        ...
                        resolved = false;
                    beforeEach(function () {
                        $httpBackend.expectPATCH(workW222557057913Uri).respond(200, '');
                        promise = triple.save();
                    ...
                    describe("And save returns from server", function () {
                        ...
                        describe("And save is triggered without value being changed", function () {
                            beforeEach(function () {
                                resolved = false;
                                promise = triple.save();
                                promise.then(function () {
            ...
            describe("And we want to add a triple by first selecting a field", function () {
                beforeEach(function () {
                    $scope.selectedField = $scope.fields[0];
                });
                
                it("The selected field predicate should be 'http://deichman.no/ontology#Work'", function () {
                    expect($scope.selectedField.predicate).toBe('http://deichman.no/ontology#Work');
                });
                ...
                describe("And the user clicks the add button", function () {
                    ...
                    it("The selected field predicate should still be 'http://deichman.no/ontology#Work'", function () {
                        expect($scope.selectedField.predicate).toBe('http://deichman.no/ontology#Work');
                    });
            ...
            describe("And the user clicks 'remove' on the first triple (fail)", function () {
                ...
                beforeEach(function () {
                    tripleToRemove = $scope.triples[0];
                    $httpBackend.expectPATCH(workW222557057913Uri).respond(400, '');
                    $scope.removeTriple(tripleToRemove);
                //$httpBackend.flush();
                });
            ...
            describe("And the user clicks 'New Work' (success)", function () {
                beforeEach(function () {
                    $httpBackend.expectPOST(newWorkUri).respond(201, '', { 'Location': workW222557057913Uri });
                    $httpBackend.expectGET(workW222557057913Uri).respond(200, mocks.work_w222557057913);
                    $scope.newWork();
                    $httpBackend.flush();
                });
