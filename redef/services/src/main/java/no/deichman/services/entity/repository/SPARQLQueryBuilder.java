...
public final class SPARQLQueryBuilder {
    ...
    public Query constructInformationForMARC(XURI publication) {
        String q = ""
                ...
                + "     ?cs deichman:hasCataloguingSource ?cataloguingSource ."
                + "     ?cataloguingSource rdfs:label ?CSLabel ; \n"
                + "     FILTER(lang(?CSLabel) = \"no\")\n"
                + "     OPTIONAL { ?cs deichman:hasIdentifier ?CSIdentifier . }\n"
                + "  }"
    ...
    public Query retriveResourceRelationships(XURI uri) {
        String queryString = format(""
                ...
                + "prefix deich: <%1$s>\n"
                + "prefix role: <%2$s>\n"
                + "select distinct ?relation ?targetUri ?mainTitle ?subtitle ?partTitle ?partNumber ?publicationYear ?type ?isMainEntry \n"
                + "where {\n"
                + "  {\n"
