...
public final class SPARQLQueryBuilder {
    ...
    public Query describeWorkAndLinkedResources(XURI xuri) {
        String queryString = "#\n"
                ...
                + "DESCRIBE <__WORKURI__> ?publication ?workContributor ?format ?mediaType ?subject ?genre ?litform ?serial ?nation ?pubContrib ?publicationContributor ?place\n"
                + "WHERE {\n"
                + "        { <__WORKURI__> a deichman:Work }\n"
                + "  UNION { <__WORKURI__> deichman:mainTitle ?title }\n"
                + "  UNION { { <__WORKURI__> deichman:contributor ?workContrib .\n"
                + "           ?workContrib a deichman:Contribution ;\n"
                + "             deichman:agent ?workContributor . }\n"
                + "           UNION { ?workContributor deichman:nationality ?nation }"
                + "    }\n"
                + "  UNION { { ?publication deichman:publicationOf <__WORKURI__> ; \n"
                + "             a deichman:Publication . }\n"
                + "          UNION { ?publication deichman:format ?format .\n"
                + "                  ?format rdfs:label ?label . }\n"
                + "          UNION { ?publication deichman:hasMediaType ?mediaType .\n"
                + "                  ?mediaType rdfs:label ?label . }\n"
                + "          UNION { ?publication deichman:contributor ?pubContrib. \n"
                + "                  ?pubContrib a deichman:Contribution ;\n"
                + "                    deichman:agent ?publicationContributor . }\n"
                + "          UNION { ?publication deichman:inSerial ?serialIssue . \n"
                + "                   ?serialIssue deichman:serial ?serial . }\n"
                + "          UNION { ?publication deichman:hasPlaceOfPublication ?place }"
                + "    }\n"
    ...
    public Query constructInformationForMARC(XURI publication) {
        String q = ""
                ...
                + "  UNION { <__PUBLICATIONURI__> deichman:abstract ?abstract }\n"
