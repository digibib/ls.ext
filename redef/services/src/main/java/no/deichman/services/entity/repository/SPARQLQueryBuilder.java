...
public final class SPARQLQueryBuilder {
    ...
    public Query describeWorkAndLinkedResources(XURI xuri) {
        String queryString = "#\n"
                ...
                + "         ?bio ?country ?contentAdaptation ?formatAdaptation ?relatedWork ?workSeries ?workAsSubjectAgent ?subPlace ?relworkMainEntry\n"
                + "WHERE {\n"
    ...
    public Query retriveResourceRelationships(XURI uri) {
        String queryString = format(""
                ...
                + "                    BIND(xsd:boolean(true) AS ?isMainEntry) . \n"
                + "    } \n"
                + "                     BIND(?withContributor AS ?targetUri) . \n"
                + "                     BIND(xsd:boolean(true) AS ?isRole) . \n"
                + "  } UNION {\n"
