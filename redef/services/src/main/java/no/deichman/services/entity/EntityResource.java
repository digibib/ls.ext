...
public final class EntityResource extends ResourceBase {
    ...
    public Response create(@PathParam("type") String type, @PathParam("id") String id, String body) throws Exception {
        ...
        if (getEntityService().resourceExists(xuri)) {
            status = Response.Status.CONFLICT;
        } else if ("work".equals(type) || "person".equals(type) || "serial".equals(type) || "genre".equals(type) || "subject".equals(type) || "place".equals(type)) {
            Model model = RDFModelUtil.modelFrom(body, Lang.NTRIPLES);
            getEntityService().create(model);
        } else {
            errMessage = Optional.of("only person|work|serial|genre|subject|place resources can be created with user assigned ID");
            status = Response.Status.BAD_REQUEST;
        }
