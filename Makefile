...
cuke_test:
	@vagrant ssh vm-test -c "cd vm-test && \
	  rm -rf report/rerun.txt && \
		$(BROWSER_ARG) $(PLACK_ARGS) cucumber --profile rerun $(CUKE_PROFILE_ARG) $(CUKE_ARGS) || \
...
cuke_redef:
	@ vagrant ssh vm-test -c "cd vm-test && \
	  rm -rf report/rerun.txt && \
		$(BROWSER_ARG) $(PLACK_ARGS) cucumber --profile rerun $(CUKE_PROFILE_ARG) --tags @redef $(CUKE_ARGS) || \
