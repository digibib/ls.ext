...
cuke_test:
	@vagrant ssh vm-test -c "cd vm-test && \
	  rm -rf report/rerun.txt && \
		$(BROWSER_ARG) $(PLACK_ARGS) cucumber --profile rerun `if [ -n \"$(CUKE_PROFILE_ARG)\" ]; then echo -n $(CUKE_PROFILE_ARG); else echo -n --profile default; fi` $(CUKE_ARGS) || \
		$(BROWSER_ARG) $(PLACK_ARGS) cucumber @report/rerun.txt `if [ -n \"$(CUKE_PROFILE_ARG)\" ]; then echo -n $(CUKE_PROFILE_ARG); else echo -n --profile default; fi` $(CUKE_ARGS)"
...
cuke_redef:
	@ vagrant ssh vm-test -c "cd vm-test && \
	  rm -rf report/rerun.txt && \
		$(BROWSER_ARG) $(PLACK_ARGS) cucumber --profile rerun `if [ -n \"$(CUKE_PROFILE_ARG)\" ]; then echo -n $(CUKE_PROFILE_ARG); else echo -n --profile default; fi` --tags @redef $(CUKE_ARGS) || \
		$(BROWSER_ARG) $(PLACK_ARGS) cucumber @report/rerun.txt `if [ -n \"$(CUKE_PROFILE_ARG)\" ]; then echo -n $(CUKE_PROFILE_ARG); else echo -n --profile default; fi` --tags @redef $(CUKE_ARGS)"
