...
class AddItem < IntraPage
  ...
  def add(barcode, branch_code, itemtype_code)
    @browser.text_field(:id => /^s2id_tag_952_subfield_p_[0-9]+$/).set barcode
    @browser.select_list(:id => /^s2id_tag_952_subfield_a_[0-9]+$/).select_value branch_code
    @browser.select_list(:id => /^s2id_tag_952_subfield_b_[0-9]+$/).select_value branch_code
    @browser.select_list(:id => /^s2id_tag_952_subfield_y_[0-9]+$/).select_value itemtype_code
